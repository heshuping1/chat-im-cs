...
| `/{chatId}/read-status` | GET | 查询对端已读状态 |
| `/{chatId}/pin` | PUT | 置顶 |
| `/{chatId}/mute` | PUT | 免打扰 |
| `/{chatId}/typing` | POST | 正在输入占位接口 |
...
| `peerLastReadAt` | DateTimeOffset? | 对端最后已读时间；未读过时为 `null` |

`POST /{chatId}/typing` 正在输入指示器：

- 无请求体
- 响应 `data`：`{ "chatId": "...", "userId": "..." }`
- 该接口仅作为占位，实际输入状态通过 Gateway 实时推送
...
| 端点 | 方法 | 说明 |
|---|---|---|
| `/messages/{messageId}/recall` | POST | 撤回消息 |
...
| `msg.read` | 已读回执推送（与 §10.4 结构相同） |
| `msg.recalled` | 消息撤回推送（与 §10.5 结构相同） |
...
#### `GET /customer-service/customers/service-history`
...
| `status` | short? | 否 | 线程状态过滤；缺省返回全部状态（含进行中与已结束） |
