  ...
  group('group read receipts mapper', () {
    test('parses contract dto with lastReadSeq against message seq', () {
      final receipts = parseGroupReadReceiptsPayload({
        'members': [
          ...
          {'userId': 'u-2', 'displayName': 'Bob', 'lastReadSeq': 2},
        ],
        'totalMembers': 2,
        'readCount': 1,
        'unreadCount': 1,
    ...
    test('keeps old hasRead list response compatible', () {
      final receipts = parseGroupReadReceiptsPayload([
        ...
        {'userId': 'u-2', 'displayName': '', 'hasRead': false},
      ], messageSeq: 5);
    ...
    test('excludes current sender from members and counts', () {
      final receipts = parseGroupReadReceiptsPayload(
        {
          'members': [
            {'userId': 'self-user', 'displayName': 'Me', 'lastReadSeq': 8},
            {'userId': 'u-2', 'displayName': 'Bob', 'lastReadSeq': 2},
            {
              'user': {
                'readerUserId': 'u-3',
    ...
    test('excludes current sender using platform, lpp id or display name', () {
      final receipts = parseGroupReadReceiptsPayload(
        {
          'members': [
            {
              'readerPlatformUserId': 'platform-self',
              'displayName': 'Me',
              'lastReadSeq': 8,
            },
            {
              'user': {'readerUserId': 'u-2', 'displayName': 'Bob'},
              'lastReadSeq': 8,
            },
            {'lppId': 'lpp-carol', 'displayName': 'Carol', 'lastReadSeq': 0},
          ],
          'totalMembers': 3,
          'readCount': 2,
          'unreadCount': 1,
        },
        currentUser: const GroupReadReceiptIdentity(
      ...
      expect(receipts.readCount, 1);
      expect(receipts.unreadCount, 1);
      expect(receipts.members.map((m) => m.displayName), ['Bob', 'Carol']);
    });

    test('keeps fallback unread count when receipt members are partial', () {
      final receipts = parseGroupReadReceiptsPayload(
        {
          'members': [
            {'userId': 'self-user', 'displayName': 'Me', 'lastReadSeq': 8},
            {'userId': 'u-2', 'displayName': 'Bob', 'lastReadSeq': 8},
          ],
          'totalMembers': 3,
          'readCount': 2,
          'unreadCount': 1,
      ...
      expect(receipts.readCount, 1);
      expect(receipts.unreadCount, 1);
      expect(receipts.readMembers.map((m) => m.userId), ['u-2']);
      expect(receipts.unreadMembers, isEmpty);
    });
  });
}
