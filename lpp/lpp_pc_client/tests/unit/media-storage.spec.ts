...
function cachedVideoPath({
  ...
  return join(userDataDir, "StartLink Files", accountId, conversationId, "Videos", month, `${hash}-${fileName}`);
}
...
function cachedMediaPath({
  ...
  const month = new Date().toISOString().slice(0, 7);
  const hash = createHash("sha1").update(url).digest("hex").slice(0, 16);
  return join(userDataDir, "StartLink Files", accountId, conversationId, directory, month, `${hash}-${fileName}`);
}
