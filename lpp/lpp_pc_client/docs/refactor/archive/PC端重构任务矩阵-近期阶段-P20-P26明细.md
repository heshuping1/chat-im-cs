...
| P24-BRAND-002 | App Icon Asset Closure | 闭环应用图标生效链路：以 `assets/app-icon-startlink.png` 为单一源，补齐 Windows `.ico`、macOS `.icns`、web favicon 与 macOS 开发态 Dock 图标；打包配置区分 Windows/NSIS 与 macOS 图标，避免只更新截图或 PNG 后运行态不生效。 | P2 | L2 | 已完成 |
