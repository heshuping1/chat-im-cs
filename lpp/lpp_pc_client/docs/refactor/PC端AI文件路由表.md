...
| 收到图片本地物化 | `src/renderer/media/runtime/imageMaterialization.ts` | `src/renderer/data/gateway/gateway-im-side-effects.ts`、`src/renderer/data/gateway/gateway-cs-side-effects.ts`、`src/renderer/messages/hooks/useMessageAuxiliaryData.ts`、`src/renderer/customer-service/hooks/useCustomerServiceThreadLifecycle.ts`、`src/main/media-storage.ts` | UI 组件不得作为图片落盘主入口；Gateway raw payload 必须先由 adapter/side-effect 归一化成 `MessageItemDto`；不新增 IPC，继续复用 `desktopApi.cacheMediaFile` | `npx vitest run tests/unit/media-materialization.spec.ts tests/unit/gateway-im-side-effects.spec.ts tests/unit/gateway-cs-side-effects.spec.ts tests/unit/media-storage.spec.ts tests/unit/architecture-boundaries.spec.ts` |
