...
| 消息状态与回执 | `src/renderer/data/message/message-status-model.ts` | `src/renderer/data/message/message-retry-model.ts`、`src/renderer/data/message/message-view-model.ts`、`src/renderer/data/message/group-read-receipts-model.ts`、`src/renderer/components/ChatMessageBubble.tsx`、`src/renderer/messages/components/MessageListPanel.tsx`、`src/renderer/messages/components/GroupReadReceiptPopover.tsx`、`src/renderer/customer-service/components/ServiceMessageBubble.tsx`、`src/renderer/styles/messages/message-center.css` | 群聊 read receipt 详情请求仅限点击消息旁浮层，不能参与消息主链路归约、本地库写入、`msg.read` 处理或本地 readCount 累加；失败/已读/重发规则不得写回页面组件；文本/图片使用气泡左侧状态位；视频和文件本地上传、暂停、失败、取消不走外侧状态位，分别由视频 overlay 和文件卡片承载；权限类失败不得把 403/FORBIDDEN/账号无权限直接展示给用户，真实原因只进 diagnostics | `npx vitest run tests/unit/message-status-model.spec.ts tests/unit/message-retry-model.spec.ts tests/unit/message-view-model.spec.ts tests/unit/group-read-receipts-model.spec.ts tests/unit/message-failure-marker-style.spec.ts tests/unit/message-domain.spec.ts` |
