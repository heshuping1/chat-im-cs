...
| `src/renderer/components/WorkbenchPage.tsx` | workbench page assembly | 企业工作台入口、角色卡片、模块路由卡和空态装配 | 业务数据协议、客服消息规则、权限纯规则 | 603 | 刚过页面审查线，主要是入口装配和展示 copy；立即拆分收益低 | `WorkbenchPage` 路由入口不变 | 观察保留；后续新增真实工作台数据时先迁出数据 controller |
| `src/renderer/components/ThreadList.tsx` | customer-service thread list presentation | 在线客服线程列表、筛选、状态标记和列表交互展示 | 客服 cache merge、unread ledger、Gateway 副作用 | 546 | 超组件审查线但 owner 单一；拆分前需避免列表状态和样式跳转过多 | `ThreadList` 组件入口不变 | 观察保留；新增复杂分组或批量操作时优先拆 row/filter 子组件 |
...
| `src/renderer/messages/components/MessageListPanel.tsx` | IM message list panel | 消息列表渲染、查找/历史面板、媒体 lookup 预览和列表窗口化展示 | 本地消息库 owner、query/cache 写入、发送/已读业务规则、API DTO normalizer | 657 | 超组件审查线，P27 新增 lookup 范围提示后越线；当前仍是同一列表展示 owner，立即拆分会增加查找与媒体预览跳转 | `MessageListPanel` props 入口不变 | 观察保留；后续继续增长优先拆 media lookup preview 或 lookup summary 子组件，领域判断继续留在 model/hook |
...
| `src/renderer/data/im-message-normalize.ts` | IM message normalize owner | 消息 body/type/parts/media/url/preview 归一化和兼容字段防腐 | UI 展示组件、API client 请求、Gateway 分流、cache 写入 | 650 | data owner 超审查线，历史兼容规则集中但职责仍单一；当前拆分会影响 message contract 和媒体展示多处调用 | normalize 导出入口不变 | 观察保留；新增大块媒体或联系人卡片规则时优先拆专用 normalizer |
...
| `src/renderer/styles/pages/product-pages.css` | product pages CSS | 工作台、账号辅助页、AI 控制台、收藏等产品页面样式 | IM 消息中心、客服工作台、settings 专属样式、shared shell token | 1668 | 刚过 CSS 审查线，当前仍是产品页聚合 owner；继续增长会让页面级样式定位变慢 | 产品页样式入口不变 | 观察保留；新增大块页面样式优先拆到对应 page CSS owner |
...
| `src/renderer/components/LoginPage.tsx` | auth page assembly | 登录、注册、邀请码接受、空间选择和认证 session 写入装配 | 表单基础控件、注册校验规则、Auth API 请求实现、持久化 store owner | 608 | 页面刚过审查线，当前 owner 仍集中在认证流程编排；邀请码注册闭环使页面略长 | `LoginPage` 路由入口不变 | 观察保留；后续继续增长优先抽 `useAuthSubmitController` 或邀请接受 controller |
...
| `src/renderer/data/message-store/im-message-store.ts` | IM local message store adapter | `ImMessageStore` contract、memory/IndexedDB/desktop LocalDataService adapter、P27 到 P28 迁移桥接 | 客服本地模型、媒体缓存规则、UI 展示、SQLite driver | 742 | P28 同时保留 fallback 与迁移桥接导致偏长，但 UI 仍只有一个消息本地库出口；立即拆分会增加迁移排查跳转 | `getImMessageStore` / `createDesktopImMessageStore` 入口不变 | 观察保留；新增客服本地模型、复杂搜索 projection 或媒体规则时拆 `im-message-store-desktop` / `im-message-store-migration` |
...
| `src/main/local-data/local-data-sqlite-driver.ts` | Local Data SQLite driver | SQLite schema、migration、message CRUD、FTS index、storage stats 和 diagnostics callback | 媒体文件落盘、客服 thread 领域规则、send outbox 调度、repair UI | 539 | P28 schema 与 DB 操作处于同一 driver 边界，拆分太早会分散 migration/schema owner | SQLite driver constructor 和 `LocalDataDriver` 接口不变 | 观察保留；新增媒体文件落盘、客服模型或 repair UI 时拆 repository/service |
| `src/main/screenshot-selection-template.ts` | screenshot selection template | 截图选择隔离窗口 HTML/CSS/交互脚本模板 | renderer 业务状态、文件系统能力、消息/客服领域规则 | 578 | main template 超审查线，但模板必须连续审查转义、样式和交互，机械拆分会降低安全审查完整性 | `screenshotSelectorHtml` 入口不变 | 例外保留；新增截图外能力或 IPC 时必须拆出专用 main runtime |
| `src/renderer/media/domain/mediaMessage.ts` | media message domain | 图片/视频/文件/语音等消息媒体 item、文件名、URL、预览和动作 payload 规则 | React 组件、desktopApi 调用、上传 runtime、cache 写入 | 530 | model domain 刚过审查线，媒体展示和动作 payload 规则同属媒体领域；拆分需等待规则域明显分叉 | media domain 导出入口不变 | 观察保留；新增独立媒体类型或播放策略时优先拆 `mediaPreview`/`mediaAction` model |
| `src/renderer/media/runtime/mediaMaterialization.ts` | media materialization runtime | 收到/发送后的媒体本地物化、缓存索引和桌面缓存能力编排 | UI 展示、消息发送规则、Gateway 分流、API DTO normalizer | 503 | runtime 刚过审查线，当前 owner 单一且是 IPC 能力隔离点；拆分需避免组件直接接触 desktopApi | `materializeMessageMedia` 等 runtime 入口不变 | 观察保留；继续增长优先拆 materialization index 或 cache policy helper |
