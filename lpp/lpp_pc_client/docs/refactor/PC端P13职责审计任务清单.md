...
| `src/main/screenshot-selection-template.ts` | Electron screenshot template owner | 截图选择页 HTML/CSS/交互脚本模板。 | 模板字符串较长但只服务 screenshot selector window。 | `screenshotSelectorHtml`。 | 允许例外；只可在截图选择体验或安全隔离修改时变更，不扩大 preload/renderer IPC contract。 |
...
| `src/main/screenshot-selection-template.ts` | Electron screenshot template owner | 截图选择页 HTML/CSS/交互脚本模板只能在 main-only 隔离窗口使用；拆分会降低模板安全审查连续性。 | 新增截图选择外的 UI、IPC、文件系统或 renderer 能力时，拆出专用 main template/runtime owner。 | `npx vitest run tests/unit/electron-template.spec.ts tests/unit/architecture-boundaries.spec.ts` |
...
| `src/renderer/data/gateway/gateway-event-router.ts` | Gateway event routing owner | Gateway event fan-out, first-stage routing, reminder invalidation, and customer-service/IM delivery dispatch still share one routing boundary before side-effects fan out. | Do not add cache-owner writes, legacy payload compatibility, or UI state rules here; if route branches keep growing, split per-domain router modules behind the same facade. | `npx vitest run tests/unit/gateway-event-router.spec.ts tests/unit/architecture-boundaries.spec.ts` |
...
| `src/renderer/data/customer-service/cs-contract.ts` | Customer service contract normalizer owner | Customer service DTO normalization, thread/message field coercion, and degraded-contract diagnostics remain one anti-corruption entry before domain mapping. | Do not add UI presentation, cache-owner writes, or gateway routing heuristics here; if DTO families keep growing, split thread/message normalizers behind the same contract facade. | `npx vitest run tests/unit/cs-contract.spec.ts tests/unit/architecture-boundaries.spec.ts` |
| `src/renderer/data/api/contacts-client.ts` | Contacts API client owner | Contacts/friend request request builders, response normalization, and roster DTO helpers still serve one contacts API boundary. | Do not add page state, workspace actions, or renderer presentation here; if friend loop and enterprise directory rules keep growing, split response normalizers into dedicated contacts owners. | `npx vitest run tests/unit/contacts-client.spec.ts tests/unit/architecture-boundaries.spec.ts` |
| `src/main/local-data/local-data-sqlite-driver.ts` | Local Data SQLite driver owner | SQLite schema、migration、message CRUD、FTS index、storage stats 和 diagnostics callback 属于同一 DB driver 边界；当前拆分会让迁移事务和 schema owner 分散。 | 新增媒体文件落盘、客服 thread 领域规则、send outbox 调度或 repair UI 时，不继续塞进 driver，拆到对应 repository/service。 | `npx vitest run tests/unit/local-data-sqlite-driver.spec.ts tests/unit/architecture-boundaries.spec.ts` |
| `src/renderer/data/message-store/im-message-store.ts` | IM local message store adapter owner | Memory/IndexedDB/desktop LocalDataService adapter 共享一个 `ImMessageStore` contract，保留 P27 fallback 和 P28 SQLite 迁移桥接；拆分前需保持 UI 单一出口。 | 新增客服本地模型、媒体缓存规则或复杂搜索 projection 时，不继续写入本文件，拆 `im-message-store-desktop.ts` / `im-message-store-migration.ts`。 | `npx vitest run tests/unit/im-message-store.spec.ts tests/unit/architecture-boundaries.spec.ts` |
