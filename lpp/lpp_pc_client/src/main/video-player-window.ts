...
function videoPlayerHtml(payload: {
  fileName: string;
  fileUrl: string;
  posterUrl?: string;
  title: string;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .state { position: absolute; inset: 0; display: none; place-items: center; background: rgba(236,236,236,.94); color: #4b5563; font-size: 13px; z-index: 3; }
    .state.show { display: grid; }
    .state button { width: auto; height: 34px; padding: 0 14px; border: 1px solid rgba(0,0,0,.12); background: #fff; color: #1f2937; }
  </style>
</head>
<body>
  ...
  <main class="stage">
    <div id="wrap" class="video-wrap ${payload.posterUrl ? 'has-poster' : 'no-poster'}">
      ...
      <video id="video" src="${escapeHtml(payload.fileUrl)}" ${payload.posterUrl ? `poster="${escapeHtml(payload.posterUrl)}"` : ''} playsinline preload="auto"></video>
      <div id="state" class="state"><button id="retry">&#35270;&#39057;&#21152;&#36733;&#22833;&#36133;&#65292;&#28857;&#20987;&#37325;&#35797;</button></div>
      <div class="controls">
  ...
  <script>
    ...
    document.getElementById('download').onclick = () => window.desktopApi?.saveMediaAs({ url: payload.fileUrl, fileName: payload.fileName, kind: 'video' });
    document.getElementById('retry').onclick = () => { state.classList.remove('show'); video.load(); };
    video.onplay = () => { wrap.classList.add('started'); sync(); };
    video.onpause = sync; video.ontimeupdate = sync; video.onloadedmetadata = sync; video.onended = sync;
    video.onloadeddata = () => { wrap.classList.add('ready'); sync(); };
    video.oncanplay = () => { wrap.classList.add('ready'); sync(); };
    video.onerror = () => { wrap.classList.remove('ready'); state.classList.add('show'); };
