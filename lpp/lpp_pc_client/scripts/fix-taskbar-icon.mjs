...
function updateShortcuts(targetExes, preferredTargetExe, shortcutIcon) {
  const script = [
    ...
    "if ($targets -contains $shortcut.TargetPath -or $shortcut.TargetPath -match 'StartLink|startlink|LPP|lpp' -or $_.Name -match '\\u661f\\u7edc|微界|StartLink|startlink|LPP|客服|lpp') {",
    "$shortcut.TargetPath = $target;",
    "$shortcut.WorkingDirectory = Split-Path $target;",
    "$shortcut.IconLocation = $icon;",
    "$shortcut.Save();",
    "if ($_.Name -ne $shortcutName -and $_.DirectoryName -notlike '*\\User Pinned\\TaskBar') {",
